package com.electdept.authpermissionsservice.controller;
...
import com.electdept.authpermissionsservice.service.AuthService;
...
public class AuthController {
    ...
    public ResponseEntity<?> assignRoleToUser(@PathVariable Long userId,
        ...
        try {
            Long grantedBy = (Long) auth.getPrincipal();
            String bearer = req.getHeader("Authorization");
            authService.assignRoleToUser(userId, roleId, grantedBy, bearer);
    ...
    public ResponseEntity<?> removeRoleFromUser(@PathVariable Long userId,
        ...
        try {
            Long grantedBy = (Long) auth.getPrincipal();
            String bearer = req.getHeader("Authorization");
            authService.removeRoleFromUser(userId, roleId, grantedBy, bearer);
    ...
    public ResponseEntity<?> grantWarehouseAccess(@PathVariable Long userId, @PathVariable Long warehouseId) {
        try {
            ...
            return ResponseEntity.ok(Map.of("message", "Substation access granted successfully"));
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(Map.of("message", e.getMessage()));
        }
    ...
    public ResponseEntity<?> revokeWarehouseAccess(@PathVariable Long userId, @PathVariable Long warehouseId) {
        try {
            ...
            return ResponseEntity.ok(Map.of("message", "Substation access revoked successfully"));
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(Map.of("message", e.getMessage()));
        }
