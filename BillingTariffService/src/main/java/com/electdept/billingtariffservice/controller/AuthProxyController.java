...
public class AuthProxyController {
    ...
    @RequestMapping(value = "/auth/{path:[^?]*}", method = {RequestMethod.GET, RequestMethod.POST, RequestMethod.PUT, RequestMethod.DELETE})
    public ResponseEntity<?> authProxy(@PathVariable String path, HttpServletRequest request) {
        String fullPath = "/auth/" + path;
        HttpMethod httpMethod = HttpMethod.valueOf(request.getMethod());
        
        if (httpMethod == HttpMethod.GET || httpMethod == HttpMethod.DELETE) {
            return forward(fullPath, httpMethod, null);
        } else {
            return forward(fullPath, httpMethod, null);
        }
    }
